...
glm::vec3 isColliding(const Corridor& cor){
    ...
    else if ((cor.getRW() < ballPosition[0])){
        temp = Vector(-1,0,0);
        retVal=reflectAbout(retVal,temp);
    }
    else if ((cor.getTW() < ballPosition[1])){
        temp = Vector(0,-1,0);
        retVal=reflectAbout(retVal,temp);
    }
    else if ((ballPosition[1] < cor.getBW())){
        temp = Vector(0,1,0);
        retVal=reflectAbout(retVal,temp);
    }
    else if ((ballPosition[2] > cor.getFW())){
        temp = Vector(0,0,1);
        retVal=reflectAbout(retVal,temp);
    }
    else if ((ballPosition[2] > -15.0f)){
        temp = Vector(0,0,1);
        retVal=reflectAbout(retVal,temp);
    }
...
glm::vec3 reflectAbout (const glm::vec3& aVec,const glm::vec3& bVec){
    float alpha;
